...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Pick a random roulette value
  const randomIndex = Math.floor(Math.random() * rouletteValues.length);
  const randomValue = rouletteValues[randomIndex];
  ...
  // Send the response with the random roulette value
